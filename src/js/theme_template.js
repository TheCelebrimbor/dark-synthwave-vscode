(function () {
	...
	const usingSynthwave = () => {
	  ...
	  const synthWaveTheme = document.querySelector('[class*="Celebrimbor-new-dark-synthwave-vscode-themes"]');
	...
	const initNeonDreams = (disableGlow, obs) => {
	  ...
	  console.log('New Dark Synthwave \'84: NEON DREAMS initialised!');
	  ...
	  for(let mutation of mutationsList) {
		if (mutation.type === 'attributes' || mutation.type === 'childList') {
		  ...
		  if (readyForReplacement(tokensEl, tokenReplacements)) {
			...
			observer.disconnect();
			console.log('New Dark Synthwave \'84: Cleaned up');
		  }
		  else {
			const applicationNode = document.querySelector('div[role="application"]');
			observer.observe(applicationNode, { attributes: true })
