...
function activate(context) {
	this.extensionName = 'Celebrimbor.new-dark-synthwave-vscode';
	this.cntx = context;
	...
	let disposable = vscode.commands.registerCommand('new-dark-synthwave84.enableNeon', function () {
		...
		const templateFile = path.join(base, electronBase, "workbench", "dark-neondreams.js");
		...
		try {
			...
			const isEnabled = html.includes("dark-neondreams.js");

			if (!isEnabled) {
				// delete synthwave script tag if there
				let output = html.replace(/^.*(<!-- SYNTHWAVE 84 --><script src="dark-neondreams.js"><\/script><!-- NEON DREAMS -->).*\n?/mg, '');
				// add script tag
				output = html.replace(/\<\/html\>/g, `	<!-- SYNTHWAVE 84 --><script src="dark-neondreams.js"></script><!-- NEON DREAMS -->\n`);
				output += '</html>';
	...
	let disable = vscode.commands.registerCommand('new-dark-synthwave84.disableNeon', uninstall);
...
function uninstall() {
	...
	const htmlFile = path.join(base, electronBase, "workbench", workBenchFilename);
	const neonDreamsFile = path.join(base, electronBase, "workbench", "dark-neondreams.js");
	...
	fs.rmSync(neonDreamsFile, { force: true });
}
